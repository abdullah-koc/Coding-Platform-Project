...
public class AttemptRepository {
    ...
    public void insertAttempt(Attempt attempt) {
        ...
        if (!contestId.equals("")) {
            ...
            int try_count = 0;
            if (jdbcTemplate.queryForObject(sql_try_count, Integer.class,
                    new Object[] { attempt.getUser_id(), attempt.getQuestion_id() }) != null) {
                try_count = jdbcTemplate.queryForObject(sql_try_count, Integer.class,
                        new Object[] { attempt.getUser_id(), attempt.getQuestion_id() });
            }
        ...
        if (findAttemptByAttemptId(attempt.getAttempt_id()) != null) {
            try {
                ...
                if (is_solved && !already_solved_correctly) {
                    ...
                    int point = (int) jdbcTemplate.queryForObject(find_question_point, Integer.class,
                            attempt.getQuestion_id());

                    String current_user_point = "SELECT user_point FROM users WHERE user_id = ?";
                    int user_point = (int) jdbcTemplate.queryForObject(current_user_point, Integer.class,
                            attempt.getUser_id());
