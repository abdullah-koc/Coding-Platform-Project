...
public class ContestRepository {
   ...
   public List<ContestResultDto> getContestantsByOrder(String contest_id) {
      String sql = "(SELECT nickname, SUM(question_point) points FROM (users u JOIN people p ON u.user_id = p.person_id) NATURAL JOIN user_contest NATURAL JOIN attempts NATURAL JOIN questions NATURAL JOIN question_contest WHERE is_solved = b'1' AND contest_id = ? GROUP BY nickname ORDER BY points DESC) UNION (SELECT nickname, 0 AS points FROM (users u JOIN people p ON u.user_id = p.person_id) NATURAL JOIN user_contest NATURAL JOIN attempts NATURAL JOIN questions NATURAL JOIN question_contest WHERE contest_id = ? AND nickname NOT IN (SELECT nickname FROM (users u JOIN people p ON u.user_id = p.person_id) NATURAL JOIN user_contest NATURAL JOIN attempts NATURAL JOIN questions NATURAL JOIN question_contest WHERE is_solved = b'1' AND contest_id = ?) )";
      return jdbcTemplate.query(sql, (rs, rowNum) -> {
