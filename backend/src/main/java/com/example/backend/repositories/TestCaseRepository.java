...
public class TestCaseRepository {
    ...
    public void insertTestCase(TestCase testCase) {
        ...
        try {
            last_test_case_id_of_question = (String) jdbcTemplate.queryForObject(last_test_case_id_sql,
                    new Object[] { testCase.getCoding_question_id() }, String.class);
        ...
        jdbcTemplate.update(sql, test_case_id, testCase.getExample_input(), testCase.getExample_output(),
                testCase.isIs_locked(), testCase.getCoding_question_id());
    }
    ...
    public List<TestCase> findTestCases(String codingQuestionId) {
        ...
        try {
            return jdbcTemplate.query(sql, new Object[] { codingQuestionId },
                    new BeanPropertyRowMapper(TestCase.class));
        } catch (EmptyResultDataAccessException e) {
            return null;
        }
    }

    public List<AttemptTestCaseDto> getAllAttemptTestCaseMatches(String question_id, String user_id,
            String attempt_id) {
        try {
            String sql = "SELECT * FROM attempt_test_case WHERE coding_question_id = ? AND user_id = ? AND attempt_id = ?";
            return jdbcTemplate.query(sql, new Object[] { question_id, user_id, attempt_id },
                    new BeanPropertyRowMapper(AttemptTestCaseDto.class));
    ...
    public TestCase findTestCaseById(String coding_question_id, String test_case_id) {
        try {
            ...
            return jdbcTemplate.queryForObject(sql, new Object[] { coding_question_id, test_case_id },
                    new BeanPropertyRowMapper<>(TestCase.class));
        } catch (EmptyResultDataAccessException e) {
            return null;
        }
