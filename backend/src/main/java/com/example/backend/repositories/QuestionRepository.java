...
public class QuestionRepository {
   ...
   public void insertQuestion(QuestionDto question) {
      ...
      if (question.getQuestion_id().substring(0, 3).equals("NCQ")) {
         // update type description
         sql = "UPDATE questions SET type_description = ? WHERE question_id = ?";
         jdbcTemplate.update(sql, question.getType_description(), question.getQuestion_id());
      }
   }
